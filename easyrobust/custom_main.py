...
def main(args, args_text):
    ...
    if args.model.startswith('swin_transformer'):
        import easyrobust.models.swin_transformer as swin_transformer
    ...
    if args.model.startswith('swin_transformer'):
        from easyrobust.optimizers.optimizer import build_optimizer
...
def train_one_epoch(
    ...
    if args.training_mode == 'autoattack':
        autoattck = AutoAttack(model,norm="Linf",steps=args.attack_steps, query=args.n_queries, eps=.3, version=args.attack_version,device=args.gpu)
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        elif args.training_mode == 'autoattack':
            input = autoattck.run_standard_evaluation(input, target,bs=len(input))
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if args.training_mode in ['advtrain', 'advprop', 'pyramid_advtrain', 'autoattack']:
            if args.training_mode == 'autoattack':
                advinput = autoattck.run_standard_evaluation(input, target,bs=len(input))
            else:
